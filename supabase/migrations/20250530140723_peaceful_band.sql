...
INSERT INTO orders (user_id, order_id, customer_name, customer_email, customer_phone, status, total_amount, shipping_address, payment_method) VALUES
((SELECT id FROM auth.users WHERE email = 'admin@missorgano.com'), 'ORD001', 'Emma Johnson', 'emma@example.com', '123-456-7890', 'completed', 96.00, 
'{"street": "123 Nature St", "city": "Green City", "state": "GC", "zip_code": "12345", "country": "United States"}', 'cod'),

((SELECT id FROM auth.users WHERE email = 'admin@missorgano.com'), 'ORD002', 'Sophie Williams', 'sophie@example.com', '234-567-8901', 'processing', 78.00,
'{"street": "456 Organic Ave", "city": "Eco Town", "state": "ET", "zip_code": "67890", "country": "United States"}', 'card'),

((SELECT id FROM auth.users WHERE email = 'admin@missorgano.com'), 'ORD003', 'Michael Chen', 'michael@example.com', '345-678-9012', 'pending', 124.00,
...
((SELECT id FROM orders WHERE order_id = 'ORD001'), 1, 2, 48.00),
((SELECT id FROM orders WHERE order_id = 'ORD002'), 2, 1, 38.00),
((SELECT id FROM orders WHERE order_id = 'ORD002'), 3, 1, 24.00),
((SELECT id FROM orders WHERE order_id = 'ORD003'), 4, 2, 36.00),
((SELECT id FROM orders WHERE order_id = 'ORD003'), 5, 1, 42.00);